...
export async function monitoringRoutes(fastify: FastifyInstance, _opts: FastifyPluginOptions) {
  ...
  fastify.get('/overview', async (_request, reply) => {
    try {
      ...
      const [stationRows] = await pool.query<RowDataPacket[]>(
        `SELECT id, code, name, location, status FROM stations WHERE code = 'BATANG' LIMIT 1;`
      );
      const station = stationRows[0] || null;

      // Latest station measurements (pressure & flow)
      const [latestSession] = await pool.query<RowDataPacket[]>(
      ...
      const [slotsData] = await pool.query<RowDataPacket[]>(`
        SELECT 
          ps.id AS slot_id,
          ps.slot_code,
      ...
      // Summary counts
      const [countPumps] = await pool.query<RowDataPacket[]>(`SELECT COUNT(*) as count FROM pumps;`);
      const [countSensors] = await pool.query<RowDataPacket[]>(`SELECT COUNT(*) as count FROM sensors;`);
      const [countFailures] = await pool.query<RowDataPacket[]>(`SELECT COUNT(*) as count FROM failure_events;`);
      const [countSessions] = await pool.query<RowDataPacket[]>(`SELECT COUNT(*) as count FROM measurement_sessions;`);
      ...
      const [recentSessions] = await pool.query<RowDataPacket[]>(`
        SELECT ms.id, ms.measured_at, ms.source_type, ms.data_type,
  ...
  fastify.get('/analytics', async (request, reply) => {
    try {
      const { slotCode = 'A', measurementPoint = 'PUMP-DE-H', limit = 48 } = request.query as any;
  ...
  fastify.get('/failures', async (_request, reply) => {
    try {
      const [failures] = await pool.query<RowDataPacket[]>(`
        SELECT 
          fe.id,
          p.asset_code,
          p.name as pump_name,
  ...
  fastify.get('/assets', async (_request, reply) => {
    try {
      ...
      // Sensors Catalog
      const [sensors] = await pool.query<RowDataPacket[]>(`
        SELECT 
          s.id,
          s.pump_id,
          p.asset_code,
  ...
  fastify.get('/imports', async (_request, reply) => {
    try {
      ...
      const [provenanceStats] = await pool.query<RowDataPacket[]>(`
  ...
  fastify.post('/replace-sensor', async (request, reply) => {
    const { oldSensorId, newSerialNumber, notes } = request.body as any;
    if (!oldSensorId) {
    ...
    try {
      ...
      const [rows] = await conn.query<RowDataPacket[]>(`SELECT * FROM sensors WHERE id = ? LIMIT 1;`, [oldSensorId]);
      if (!rows.length) {
        conn.release();
  ...
  fastify.post('/swap-pump', async (request, reply) => {
    const { slotId, newAssetCode, serialNumber, pumpName, manufacturer, model, notes } = request.body as any;
    ...
    try {
      ...
      const [currentInst] = await conn.query<RowDataPacket[]>(`
      ...
      let targetPumpId: number;
      const [existingPump] = await conn.query<RowDataPacket[]>(`
      ...
      if (existingPump.length) {
        ...
        const [pRes] = await conn.query<ResultSetHeader>(`
  ...
  fastify.post('/measurements/manual', async (request, reply) => {
    const { sensorId, value, notes, quality = 'GOOD' } = request.body as any;
    ...
    try {
      ...
      const [msrRes] = await conn.query<ResultSetHeader>(`
  ...
  fastify.put('/measurements/:id', async (request, reply) => {
    ...
    const { value, quality = 'GOOD', reason } = request.body as any;
  ...
  fastify.post('/qr/resolve', async (request, reply) => {
    const { qrCode } = request.body as any;
    if (!qrCode) {
    ...
    try {
      ...
      if (!sensorRows.length) {
        ...
        const [retiredRows] = await pool.query<RowDataPacket[]>(`
        ...
        const [pumpRows] = await pool.query<RowDataPacket[]>(`
      ...
      const [lastMeasRows] = await pool.query<RowDataPacket[]>(`
  ...
  fastify.post('/operator/skip', async (request, reply) => {
    const { sensorId, slotCode, pointCode, reason = 'Pump not running' } = request.body as any;
  ...
  fastify.post('/operator/report-sensor-issue', async (request, reply) => {
    const { sensorId, slotCode, pointCode, issueType, notes } = request.body as any;
  ...
  fastify.get('/sensors-summary', async (_request, reply) => {
    try {
      ...
      const [latestReadings] = await pool.query<RowDataPacket[]>(`
      ...
      const readingMap = new Map<number, { value: number; quality: string; measured_at: string }>();
      ...
      const formatted = sensors.map((s) => {
        ...
        let status: 'NORMAL' | 'WARNING' | 'CRITICAL' | 'STANDBY' = 'NORMAL';
        if (slot === 'B' || slot === 'D') {
          status = 'STANDBY';
  ...
  fastify.post('/manual-measurement', async (request, reply) => {
    const { slotCode = 'C', measuredAt, operatorName, notes, measurements } = request.body as any;
    ...
    try {
      ...
      const [pumpSensors] = await conn.query<RowDataPacket[]>(`
        SELECT id, measurement_point, measurement_type FROM sensors WHERE pump_id = ?;
      `, [pumpId]);

      const sensorPointMap = new Map<string, number>();
      ...
      for (const [key, val] of Object.entries(measurements)) {
        ...
        if (sensorId && val !== undefined && val !== null) {
          await conn.query(`
            INSERT INTO sensor_measurements (measurement_session_id, sensor_id, value, quality)
            VALUES (?, ?, ?, 'GOOD');
          `, [sessionId, sensorId, parseFloat(val as string) || 0]);
          insertedCount++;
        }
      }
