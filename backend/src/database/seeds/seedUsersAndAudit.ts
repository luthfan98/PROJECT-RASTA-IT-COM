  ...
  try {
    ...
    for (const u of usersToSeed) {
      await connection.query(
        `INSERT INTO users (username, name, email, password, role, status, last_login_at)
         VALUES (?, ?, ?, ?, ?, ?, ?)
        ...
        [u.username, u.name, u.email, u.password, u.role, u.status, u.last_login_at]
      );
    }
    console.log('✅ Users seeded successfully.');

    // Fetch user IDs
    const [userRows]: any = await connection.query('SELECT id, username, name, role FROM users;');
    const userMap: Record<string, { id: number; name: string; role: string }> = {};
