...
export async function seedMasterSensors() {
    ...
    `);
        const [stationRows] = await conn.query(`SELECT id FROM stations WHERE code = 'BATANG' LIMIT 1;`);
      ...
      `, [stationId, code, `Booster Pump Slot ${code}`, `Posisi operasi pompa slot ${code} di Batang Station`]);
            const [sRows] = await conn.query(`SELECT id FROM pump_slots WHERE station_id = ? AND slot_code = ? LIMIT 1;`, [stationId, code]);
        ...
        const physicalPumpMap = new Map(); // slotCode -> pumpId
      ...
      `, [assetCode, unit.serial, pumpName, qrCode]);
            const [pRows] = await conn.query(`SELECT id FROM pumps WHERE asset_code = ? LIMIT 1;`, [assetCode]);
            ...
            const [existingInstall] = await conn.query(`
