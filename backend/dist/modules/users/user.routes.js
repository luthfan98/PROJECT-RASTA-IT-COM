...
export async function userRoutes(fastify, _opts) {
    ...
    fastify.get('/:id', async (request, reply) => {
        ...
        try {
            const [rows] = await pool.query(`SELECT id, username, name, email, role, status, last_login_at, created_at, updated_at FROM users WHERE id = ? LIMIT 1`, [id]);
            if (rows.length === 0) {
                return reply.code(404).send({ success: false, message: 'User tidak ditemukan' });
            }
            const user = rows[0];
            // Operational statistics (Today & All Time)
            const [measurementStats] = await pool.query(`
        SELECT 
      ...
      `, [id]);
            ...
            const [recentMeasurements] = await pool.query(`
        SELECT 
          ms.id as session_id,
          ms.measured_at,
          sm.value,
    ...
    fastify.post('/', async (request, reply) => {
        const { username, name, email, role, password, confirmPassword, status = 'active' } = request.body;
        ...
        try {
            ...
            const [exist] = await pool.query('SELECT id FROM users WHERE username = ? LIMIT 1', [username.trim()]);
            if (exist.length > 0) {
