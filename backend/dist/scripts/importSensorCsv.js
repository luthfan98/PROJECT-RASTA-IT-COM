...
export async function importSensorCsv() {
    ...
    try {
        ...
        const [stations] = await conn.query("SELECT id FROM stations WHERE code = 'BATANG' LIMIT 1;");
        if (!stations.length) {
    ...
    `, [stationId]);
        ...
        const [sensors] = await conn.query("SELECT id, pump_id, measurement_point FROM sensors WHERE status = 'ACTIVE';");
    ...
    `, ['RASTA_Training_1Year_Hourly_Final.csv', 'RASTA_Training_1Year_Hourly_Final.csv', adminId]);
        ...
        let stationMeasurementsBatch = [];
        let pumpLogsBatch = [];
        let sensorMeasurementsBatch = [];
        const failureTrackers = new Map();
        ...
        const flushStationMeasurements = async () => {
            if (stationMeasurementsBatch.length === 0)
                return;
            ...
            await conn.query(`INSERT INTO station_measurements (measurement_session_id, station_id, pressure_psi, flow_value, flow_unit) VALUES ${placeholders}`, values);
            stationMeasurementsBatch = [];
        };
        const flushPumpLogs = async () => {
            if (pumpLogsBatch.length === 0)
                return;
            const placeholders = pumpLogsBatch.map(() => '(?, ?, ?, ?, ?, ?, ?, ?, ?)').join(',');
            const values = [];
            for (const row of pumpLogsBatch) {
                values.push(row.sessionId, row.slotId, row.pumpId, row.seq, row.status, row.state, row.load, row.runningHours, row.startCount);
            }
            await conn.query(`INSERT INTO pump_operating_logs (
        ...
        const flushSensorMeasurements = async () => {
            if (sensorMeasurementsBatch.length === 0)
                return;
            ...
            await conn.query(`INSERT INTO sensor_measurements (measurement_session_id, sensor_id, value, quality) VALUES ${placeholders}`, values);
        ...
        `, [stationId, timestamp, importId, adminId]);
            ...
            if (eventId && eventId.startsWith('EVT-')) {
                if (!failureTrackers.has(eventId)) {
                    ...
                    if (opState === 'DEGRADING' && !tracker.degradationStartedAt)
                        tracker.degradationStartedAt = timestamp;
                    if (opState === 'WARNING' && !tracker.warningStartedAt)
                        tracker.warningStartedAt = timestamp;
                    if (opState === 'CRITICAL' && !tracker.criticalStartedAt)
                        tracker.criticalStartedAt = timestamp;
                    if (opState === 'FAILURE' && !tracker.failureAt)
                        tracker.failureAt = timestamp;
                }
            }
        }
        // Final flushes
